...
import { CheckCircle, ArrowRight, Phone, Monitor, DollarSign, Shield, Clock, BarChart3, MapPin } from 'lucide-react';
...
const TechnologyBookkeeping = () => {
  ...
  const faqSchema = {
    ...
    "mainEntity": [
      { "@type": "Question", "name": "Do you handle SaaS revenue recognition for startups?", "acceptedAnswer": { "@type": "Answer", "text": "Yes, ClearLedger implements ASC 606-compliant revenue recognition for SaaS companies, including deferred revenue, subscription billing, and multi-element arrangement accounting. We serve tech companies across Texas, Florida, Oklahoma, and nationwide." } },
      { "@type": "Question", "name": "Can you help with R&D tax credits for tech companies?", "acceptedAnswer": { "@type": "Answer", "text": "We document and track qualifying R&D activities and expenses to support federal and state R&D tax credit claims, helping tech companies save significantly on taxes." } },
  ...
  const pageSchema = combineSchemas(
    getProfessionalServiceSchema({ name: "Technology & SaaS Bookkeeping | ClearLedger Solutions", description: "SaaS revenue recognition, R&D tax credits, and startup bookkeeping. Serving Dallas TX & nationwide.", url: "/technology-bookkeeping", category: "Technology Bookkeeping" }),
  ...
  return (
    <>
      <SEO title="Technology & SaaS Bookkeeping | Startup Accounting | ClearLedger" description="Expert tech company bookkeeping nationwide. SaaS revenue recognition, R&D tax credits, startup accounting, investor reporting & burn rate analysis. Serving Dallas TX & more. Call (903) 815-9488." keywords="technology bookkeeping, SaaS accounting, startup bookkeeping, R&D tax credits, tech company bookkeeper, investor reporting, revenue recognition, startup accounting Texas" canonical="/technology-bookkeeping" schema={pageSchema} />
      <Header />
      <main className="min-h-screen bg-background">
        <section className="pt-20 sm:pt-24 lg:pt-28 pb-12 sm:pb-16 bg-gradient-to-br from-background via-accent/10 to-background"><div className="container mx-auto px-4 sm:px-6"><div className="max-w-5xl mx-auto text-center">
          ...
          <h1 className="font-heading text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold text-foreground mb-6 leading-tight">Expert{' '}<span className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-primary-dark">Tech & SaaS Bookkeeping</span>{' '}Services</h1>
          <p className="text-base sm:text-lg md:text-xl text-muted-foreground leading-relaxed mb-8 max-w-3xl mx-auto">From seed-stage startups to scaling SaaS companies, ClearLedger provides the specialized financial management tech companies need — serving businesses across Dallas-Fort Worth, Texas, and nationwide.</p>
        ...
        <section className="py-16 bg-gradient-subtle"><div className="container mx-auto px-4"><h2 className="font-heading text-3xl lg:text-4xl font-bold text-foreground mb-12 text-center">Serving Tech Companies Nationwide</h2><div className="flex flex-wrap justify-center gap-3 max-w-3xl mx-auto">{serviceAreas.map((area, i) => (<div key={i} className="flex items-center space-x-2 bg-card rounded-full px-4 py-2 shadow-card"><MapPin className="w-4 h-4 text-primary flex-shrink-0" /><span className="text-sm font-medium text-foreground">{area}</span></div>))}</div></div></section>
        <section className="py-16 bg-background"><div className="container mx-auto px-4"><h2 className="font-heading text-3xl lg:text-4xl font-bold text-foreground mb-12 text-center">Tech Bookkeeping FAQs</h2><div className="max-w-3xl mx-auto space-y-6">{faqSchema.mainEntity.map((faq,i) => (<div key={i} className="bg-card rounded-xl p-6 shadow-card"><h3 className="font-heading text-lg font-bold text-foreground mb-2">{faq.name}</h3><p className="text-muted-foreground">{faq.acceptedAnswer.text}</p></div>))}</div></div></section>
        <section className="py-16 bg-gradient-subtle"><div className="container mx-auto px-4"><div className="bg-card rounded-2xl p-8 shadow-premium text-center"><h2 className="font-heading text-2xl lg:text-3xl font-bold text-foreground mb-4">Ready to Level Up Your Startup Finances?</h2><p className="text-muted-foreground mb-6 max-w-2xl mx-auto">Join tech companies across the country that trust ClearLedger for investor-ready financials.</p><div className="flex flex-col sm:flex-row gap-3 justify-center"><Link to="/contact"><Button size="lg" className="btn-primary text-base px-6 py-3">Schedule Free Consultation <ArrowRight className="w-4 h-4 ml-2" /></Button></Link><a href={`tel:${COMPANY_INFO.phone}`}><Button size="lg" variant="outline" className="btn-secondary text-base px-6 py-3">Call {COMPANY_INFO.phoneLocal}</Button></a></div></div></div></section>
      </main>
      <Footer />
    </>
  );
