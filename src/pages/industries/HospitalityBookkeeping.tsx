...
import { CheckCircle, ArrowRight, Phone, Hotel, DollarSign, Shield, Clock, BarChart3, MapPin } from 'lucide-react';
...
const HospitalityBookkeeping = () => {
  ...
  const faqSchema = {
    ...
    "mainEntity": [
      { "@type": "Question", "name": "Do you handle hotel occupancy tax?", "acceptedAnswer": { "@type": "Answer", "text": "Yes, ClearLedger tracks and files state and local hotel occupancy taxes for hotels and short-term rentals across Texas, Florida, Louisiana, Oklahoma, and all our service areas." } },
      ...
      { "@type": "Question", "name": "What states do you serve for hospitality bookkeeping?", "acceptedAnswer": { "@type": "Answer", "text": "ClearLedger serves hospitality businesses across Texas (including Dallas-Fort Worth), Oklahoma, Louisiana, Florida, Mississippi, New Mexico, and Panama." } }
    ]
  };
  const pageSchema = combineSchemas(
    getProfessionalServiceSchema({ name: "Hospitality & Tourism Bookkeeping | ClearLedger Solutions", description: "Hotel revenue management, OTA reconciliation, and hospitality accounting. Serving Dallas TX & nationwide.", url: "/hospitality-bookkeeping", category: "Hospitality Bookkeeping" }),
  ...
  return (
    <>
      <SEO title="Hospitality & Hotel Bookkeeping | Tourism Accounting | ClearLedger" description="Expert hospitality bookkeeping nationwide. Hotel revenue management, OTA reconciliation, occupancy tax, event accounting & seasonal forecasting. Serving Dallas TX & more. Call (903) 815-9488." keywords="hospitality bookkeeping, hotel accounting, tourism bookkeeping, OTA reconciliation, hotel occupancy tax, event venue bookkeeping, resort accounting, hospitality accounting Texas, hotel bookkeeper Florida" canonical="/hospitality-bookkeeping" schema={pageSchema} />
      <Header />
      <main className="min-h-screen bg-background">
        <section className="pt-20 sm:pt-24 lg:pt-28 pb-12 sm:pb-16 bg-gradient-to-br from-background via-accent/10 to-background"><div className="container mx-auto px-4 sm:px-6"><div className="max-w-5xl mx-auto text-center">
          ...
          <h1 className="font-heading text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold text-foreground mb-6 leading-tight">Expert{' '}<span className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-primary-dark">Hospitality Bookkeeping</span>{' '}Services</h1>
          <p className="text-base sm:text-lg md:text-xl text-muted-foreground leading-relaxed mb-8 max-w-3xl mx-auto">Multi-revenue stream management for hotels, resorts, tour operators, and event venues across Dallas-Fort Worth, Texas, and nationwide.</p>
        ...
        <section className="py-16 bg-gradient-subtle"><div className="container mx-auto px-4"><h2 className="font-heading text-3xl lg:text-4xl font-bold text-foreground mb-12 text-center">Serving Hospitality Businesses Nationwide</h2><div className="flex flex-wrap justify-center gap-3 max-w-3xl mx-auto">{serviceAreas.map((area, i) => (<div key={i} className="flex items-center space-x-2 bg-card rounded-full px-4 py-2 shadow-card"><MapPin className="w-4 h-4 text-primary flex-shrink-0" /><span className="text-sm font-medium text-foreground">{area}</span></div>))}</div></div></section>
        <section className="py-16 bg-background"><div className="container mx-auto px-4"><h2 className="font-heading text-3xl lg:text-4xl font-bold text-foreground mb-12 text-center">Hospitality Bookkeeping FAQs</h2><div className="max-w-3xl mx-auto space-y-6">{faqSchema.mainEntity.map((faq,i) => (<div key={i} className="bg-card rounded-xl p-6 shadow-card"><h3 className="font-heading text-lg font-bold text-foreground mb-2">{faq.name}</h3><p className="text-muted-foreground">{faq.acceptedAnswer.text}</p></div>))}</div></div></section>
        <section className="py-16 bg-gradient-subtle"><div className="container mx-auto px-4"><div className="bg-card rounded-2xl p-8 shadow-premium text-center"><h2 className="font-heading text-2xl lg:text-3xl font-bold text-foreground mb-4">Ready to Streamline Your Hospitality Finances?</h2><p className="text-muted-foreground mb-6 max-w-2xl mx-auto">Join hospitality businesses across the country that trust ClearLedger for expert financial management.</p><div className="flex flex-col sm:flex-row gap-3 justify-center"><Link to="/contact"><Button size="lg" className="btn-primary text-base px-6 py-3">Schedule Free Consultation <ArrowRight className="w-4 h-4 ml-2" /></Button></Link><a href={`tel:${COMPANY_INFO.phone}`}><Button size="lg" variant="outline" className="btn-secondary text-base px-6 py-3">Call {COMPANY_INFO.phoneLocal}</Button></a></div></div></div></section>
      </main>
      <Footer />
    </>
  );
