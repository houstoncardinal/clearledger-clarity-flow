...
import { CheckCircle, ArrowRight, Phone, HardHat, DollarSign, Shield, Clock, BarChart3 } from 'lucide-react';
...
const ConstructionBookkeeping = () => {
  ...
  const faqSchema = {
    ...
    "mainEntity": [
      { "@type": "Question", "name": "What is job costing in construction bookkeeping?", "acceptedAnswer": { "@type": "Answer", "text": "Job costing tracks all expenses (labor, materials, subcontractors, equipment) by individual project so you know exactly how profitable each job is. ClearLedger sets this up in QuickBooks with custom cost codes." } },
      { "@type": "Question", "name": "Do you handle AIA billing for contractors in Dallas?", "acceptedAnswer": { "@type": "Answer", "text": "Yes, we prepare AIA G702/G703 billing applications, track retainage, and manage the entire progress billing cycle for general contractors and subcontractors." } },
      { "@type": "Question", "name": "How do you track subcontractor compliance?", "acceptedAnswer": { "@type": "Answer", "text": "We maintain W-9 files, track insurance certificate expirations, manage lien waiver collection, and prepare year-end 1099-NEC forms for all subcontractors." } }
    ]
  };

  const pageSchema = combineSchemas(
    getProfessionalServiceSchema({ name: "Construction Bookkeeping Services Dallas TX", description: "Job costing, progress billing, and financial management for contractors in Dallas TX.", url: "/construction-bookkeeping", category: "Construction Bookkeeping" }),
  ...
  return (
    <>
      <SEO title="Construction Bookkeeping Services Dallas TX | Job Costing & AIA Billing | ClearLedger" description="Expert construction bookkeeping in Dallas TX. Job costing, AIA progress billing, retainage tracking, subcontractor 1099s & certified payroll. Call (903) 815-9488." keywords="construction bookkeeping Dallas TX, contractor accounting, job costing QuickBooks, AIA billing, retainage tracking, subcontractor management, construction payroll Dallas" canonical="/construction-bookkeeping" schema={pageSchema} />
      <Header />
          ...
          <div className="container mx-auto px-4 sm:px-6">
            <div className="max-w-5xl mx-auto text-center">
              <div className="inline-flex items-center space-x-2 bg-accent/90 backdrop-blur-sm px-4 sm:px-6 py-2 sm:py-3 rounded-full mb-6 border border-accent-foreground/20 shadow-sm">
                <HardHat className="w-4 h-4 sm:w-5 sm:h-5 text-accent-foreground flex-shrink-0" />
                <span className="text-xs sm:text-sm font-semibold text-accent-foreground">Construction & Contractor Bookkeeping</span>
              </div>
              <h1 className="font-heading text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold text-foreground mb-6 leading-tight">
                Dallas TX{' '}<span className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-primary-dark">Construction Bookkeeping</span>{' '}Experts
              </h1>
              <p className="text-base sm:text-lg md:text-xl text-muted-foreground leading-relaxed mb-8 max-w-3xl mx-auto">
                From job costing to AIA billing, ClearLedger provides the specialized accounting contractors need to stay profitable and bonding-ready.
              </p>
              <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center">
                <Link to="/consultation"><Button size="lg" className="btn-primary text-base sm:text-lg px-6 sm:px-8 py-3 sm:py-4">Free Contractor Consultation <ArrowRight className="w-4 h-4 sm:w-5 sm:h-5 ml-2" /></Button></Link>
