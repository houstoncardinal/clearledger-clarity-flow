...
import { CheckCircle, ArrowRight, Phone, Stethoscope, DollarSign, Shield, Clock, BarChart3 } from 'lucide-react';
...
const MedicalBookkeeping = () => {
  ...
  const faqSchema = {
    ...
    "mainEntity": [
      ...
      { "@type": "Question", "name": "Do you work with dental practices in Dallas?", "acceptedAnswer": { "@type": "Answer", "text": "Yes, we serve both medical and dental practices across Dallas-Fort Worth with specialized bookkeeping that understands the unique revenue cycle of healthcare providers." } }
    ]
  };

  const pageSchema = combineSchemas(
    getProfessionalServiceSchema({ name: "Medical & Dental Bookkeeping Services Dallas TX", description: "HIPAA-aware bookkeeping for medical and dental practices in Dallas TX.", url: "/medical-bookkeeping", category: "Medical Bookkeeping" }),
  ...
  return (
    <>
      <SEO title="Medical & Dental Bookkeeping Dallas TX | Healthcare Accounting | ClearLedger" description="Expert medical bookkeeping in Dallas TX. Insurance reconciliation, patient billing, HIPAA-compliant records, medical payroll & multi-provider tracking. Call (903) 815-9488." keywords="medical bookkeeping Dallas TX, dental practice accounting, healthcare bookkeeping, doctor bookkeeper Dallas, dentist bookkeeper Texas, medical practice financial management, insurance reconciliation" canonical="/medical-bookkeeping" schema={pageSchema} />
      <Header />
      <main className="min-h-screen bg-background">
        <section className="pt-20 sm:pt-24 lg:pt-28 pb-12 sm:pb-16 bg-gradient-to-br from-background via-accent/10 to-background">
          <div className="container mx-auto px-4 sm:px-6"><div className="max-w-5xl mx-auto text-center">
            ...
            <h1 className="font-heading text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold text-foreground mb-6 leading-tight">Dallas TX{' '}<span className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-primary-dark">Medical & Dental Bookkeeping</span>{' '}Experts</h1>
            <p className="text-base sm:text-lg md:text-xl text-muted-foreground leading-relaxed mb-8 max-w-3xl mx-auto">Specialized financial management for doctors, dentists, clinics, and healthcare providers across Dallas-Fort Worth.</p>
        ...
        <section className="py-16 bg-gradient-subtle"><div className="container mx-auto px-4"><h2 className="font-heading text-3xl lg:text-4xl font-bold text-foreground mb-12 text-center">Medical Bookkeeping FAQs</h2><div className="max-w-3xl mx-auto space-y-6">{faqSchema.mainEntity.map((faq,i) => (<div key={i} className="bg-card rounded-xl p-6 shadow-card"><h3 className="font-heading text-lg font-bold text-foreground mb-2">{faq.name}</h3><p className="text-muted-foreground">{faq.acceptedAnswer.text}</p></div>))}</div></div></section>
        <section className="py-16 bg-background"><div className="container mx-auto px-4"><div className="bg-card rounded-2xl p-8 shadow-premium text-center"><h2 className="font-heading text-2xl lg:text-3xl font-bold text-foreground mb-4">Ready to Streamline Your Practice Finances?</h2><p className="text-muted-foreground mb-6 max-w-2xl mx-auto">Join Dallas healthcare providers who trust ClearLedger.</p><div className="flex flex-col sm:flex-row gap-3 justify-center"><Link to="/contact"><Button size="lg" className="btn-primary text-base px-6 py-3">Schedule Free Consultation <ArrowRight className="w-4 h-4 ml-2" /></Button></Link><a href={`tel:${COMPANY_INFO.phone}`}><Button size="lg" variant="outline" className="btn-secondary text-base px-6 py-3">Call {COMPANY_INFO.phoneLocal}</Button></a></div></div></div></section>
      </main>
      <Footer />
    </>
  );
